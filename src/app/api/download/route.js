...
export async function GET(req) {
  try {
    ...
    // Save PDF to server
    const filePath = path.join(documentsFolder, `signed-document-${envelopeId}.pdf`);
    fs.writeFileSync(filePath, pdfBytes, "binary"); // store as binary

    return NextResponse.json({
      success: true,
      message: `Document saved on server at ${filePath}`,
      filePath: `/documents/signed-document-${envelopeId}.pdf`,
    });
  } catch (err) {
    console.error("DocuSign download error:", err.response?.body || err.message || err);
    if (
